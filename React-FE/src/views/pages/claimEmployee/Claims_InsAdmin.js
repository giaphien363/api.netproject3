...
const Claims_InsAdmin = () => {
  ...
  const getData = async (onFinish) => {
    try {
      const res = await claimAPI.getAllInsAdmin({ pageNumber: 1 })
      saveData(res)
    } catch (error) {
  ...
  const handlePageClick = async (event) => {
    ...
    try {
      const res = await claimAPI.getAllInsAdmin({ pageNumber: nextPage })
      saveData(res)
    } catch (error) {
      setHandleError(error.response.data.detail)
    } finally {
...
export default Claims_InsAdmin
